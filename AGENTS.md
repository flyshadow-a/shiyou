...
- 2026-06-17：按当前对话修正特检策略规则弹窗、疲劳输入检查弹窗与坐标表交互（6 文件，涉及 `pages/new_special_inspection_page.py`、`pages/special_strategy_rule_dialogs.py`、`pages/output_special_strategy/inspection_tool.py` 及相关测试）：为“腿柱工作点坐标”表接入 `TableClipboardController`，X/Y 坐标列支持 Excel 式复制、粘贴、剪切和清空，首列腿柱序号保持只读且不可粘贴；将构件/节点分类弹窗标题改为“构件分类识别”“节点分类识别”，补充 `* / # / %` 规则说明和“若不输入，则根据软件模糊识别”；四类规则弹窗及计算内核同步支持 `#` 仅匹配字母、`%` 仅匹配数字，全 `*` 行继续作为占位不参与匹配；将“疲劳输入检查”弹窗最后一列表头从“原始信息”改为“文件信息”，内容改为显示缺失数据文件名，并通过弹窗宽度增加 50px 使该伸缩列加宽。已执行 `python -m pytest tests/test_special_strategy_rule_dialog.py tests/test_special_strategy_rule_exclusions.py tests/test_new_special_inspection_risk_flow.py tests/test_table_clipboard.py -q` 通过（32 passed），并执行相关 `python -m compileall` 通过。
- 2026-06-17：按当前对话追加更新风险结果页表头与列宽联调记录（2 文件，涉及 `pages/upgrade_special_inspection_result_page.py`、`tests/test_upgrade_special_inspection_result_formatting.py`）：构件风险等级明细表前两列表头由 `A/B` 改为 `JointA/JointB`；节点风险等级明细表前三列表头由 `JointA/JointB/WeldType` 改为 `JointID/Brace/JointType`；明细表和汇总表统一关闭编辑触发，避免双击表头进入编辑；明细表列宽改为固定宽度并保持横向滚动条策略，当前列宽为 `[88, 88, 108, 122, 52, 52, 190, 58, 82, 122, 136]`，加宽“失效后果等级”“倒塌分析载荷系数Rm”“Pf”“失效概率等级”以及最后一列“构件风险等级/节点风险等级”，确保表头完整显示，表格超出当前框时可左右滚动。已执行 `python -m pytest tests/test_upgrade_special_inspection_result_formatting.py -q` 通过（9 passed），并执行相关 `python -m compileall` 通过。
- 2026-06-17：修复进入“更新风险结果”页面时 MySQL `special_strategy_result_snapshots` 大快照读取触发 `Out of memory` 的问题（2 文件，涉及 `services/special_strategy_state_db.py`、`tests/test_special_strategy_state_indexes.py`）：将 `load_latest_strategy_result_snapshot()` 与 `load_strategy_result_snapshot_by_run()` 从 `SELECT *` 改为先读取不含 `result_json` 的元数据，再按 `CHAR_LENGTH + SUBSTRING` 以 256KB 分块读取 `LONGTEXT result_json` 并拼回 JSON，避免服务端一次性为大字段分配约 10MB+ 结果内存；新增回归测试模拟 `SELECT *` 抛出内存错误但分块读取成功。已执行 `python -m pytest tests/test_special_strategy_state_indexes.py tests/test_upgrade_special_inspection_result_formatting.py -q` 通过（13 passed），并执行相关 `python -m compileall` 通过。

- 2026-06-17：按当前对话修正平台载荷信息页结果文件读取限制（2 文件，涉及 `pages/platform_load_information_page.py`、`tests/test_platform_load_information_async.py`）：将“操作工况最大载荷/极端工况最大载荷”等红色结果列的右键读取入口从固定识别 `psilst.factor` 改为按文件名前缀识别 `psilst*`，允许 `psilst.M1`、`psilst.lst`、`psilst.factor` 等不同后缀结果文件；同步更新菜单、文件选择过滤器、进度框与错误提示文案，保留原有解析线程和回填逻辑不变。新增 `test_import_result_accepts_psilst_prefix_file_names` 回归测试覆盖 `psilst.M1` 不再被文件名校验拦截；已执行 `python -m pytest tests/test_platform_load_information_async.py -q` 通过（18 passed），`python -m compileall pages/platform_load_information_page.py tests/test_platform_load_information_async.py` 通过。
