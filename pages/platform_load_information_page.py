...
def _normalise_overall_assessment_value(value: object) -> str:
    text = str(value or "").strip()
    return text if text in {"是", "否"} else "否"
...
def _history_rebuild_project_values(project: Dict[str, object]) -> Tuple[str, str, str]:
    name = str(project.get("project_name") or project.get("directory_name") or "").strip()
    date = str(project.get("project_year") or "").strip()
    content = str(project.get("summary_text") or "").strip()
...
def _normalise_chart_line_visibility(
...
class MultiLineChart(FigureCanvas):
    ...
    def __init__(
        ...
        self._line_visibility: Dict[str, bool] = {}
        self._legend_pick_map: Dict[object, str] = {}
        self._legend = None
        self._init_axes()
        self._hover_annots = {}
    ...
    def _legend_circle_handle(self, label: str) -> Line2D:
        visible = self._line_visibility.get(label, True)
        return Line2D(
            [0],
            [0],
            marker="o",
            linestyle="None",
            markersize=7,
            markerfacecolor="#2563eb" if visible else "#ffffff",
            markeredgecolor="#4b5563",
            markeredgewidth=1.2,
        )

    @staticmethod
    def _legend_line_handle(style: Dict[str, str]) -> Line2D:
        return Line2D(
            [0],
            [0],
            ...
            markerfacecolor=style.get("markerfacecolor", style.get("color", "#1f77b4")),
            markeredgecolor=style.get("markeredgecolor", style.get("color", "#1f77b4")),
            markersize=6.5,
            linewidth=2.0,
        )
    ...
    def _rebuild_legend(self) -> List[str]:
        ...
        for label in self._lines.keys():
            style = self._line_styles.get(label, {})
            legend_handles.append((self._legend_circle_handle(label), self._legend_line_handle(style)))
            legend_labels.append(self._display_labels.get(label, label))
        ...
        self._bind_legend_circle_pickers()
        return labels

    def _bind_legend_circle_pickers(self) -> None:
        labels = list(self._lines.keys())
        label_index = 0

        def walk(artist) -> None:
            nonlocal label_index
            if label_index >= len(labels):
                return
            children_getter = getattr(artist, "get_children", None)
            if not callable(children_getter):
                return
            children = list(children_getter())
            line_children = [child for child in children if isinstance(child, Line2D)]
            if len(line_children) >= 2 and line_children[0].get_marker() == "o":
                circle = line_children[0]
                circle.set_picker(5)
                self._legend_pick_map[circle] = labels[label_index]
                label_index += 1
                return
    ...
    def _on_legend_pick(self, event) -> None:
        label = self._legend_pick_map.get(event.artist)
        if not label:
            return
    ...
    def update_data(self, x: List[float], lines: List[Tuple[str, List[float], str]]):
        ...
        for label in existing - incoming:
            ...
            self._line_visibility.pop(label, None)
...
class PlatformLoadInformationPage(BasePage):
    ...
    def __init__(self, parent=None):
        ...
        # 缓存子计算表用户输入的数据
        self._uppercalc_saved_data: Dict[tuple[int, int], dict] = {}
    ...
    def _set_overall_assessment_combo(self, row: int, value: object = "") -> None:
        combo = QComboBox(self.table)
        combo.addItems(_overall_assessment_options())
        combo.setCurrentText(_normalise_overall_assessment_value(value))
            ...
            for i, row in enumerate(rows):
                ...
                for c, val in enumerate(row):
                    ...
                    if c == OVERALL_ASSESSMENT_COL:
                        self._set_overall_assessment_combo(rr, val)
                        continue
                    editable = (c != 0)
            ...
            for r in range(0, self.table.rowCount()):
                for c in range(self.table.columnCount()):
                    if self.table.item(r, c) is None and self.table.cellWidget(r, c) is None:
                        ...
                        if (base_rows <= r < data_end) and c == OVERALL_ASSESSMENT_COL:
                            self._set_overall_assessment_combo(r, "")
                            continue
    ...
    def _on_table_context_menu(self, pos):
        ...
        if col in _upper_block_context_menu_columns():
            menu = QMenu(self.table)
            menu.setStyleSheet(self._menu_qss())
            action = menu.addAction("打开上部组块分项目计算表")
            action.triggered.connect(
                lambda _=False, source_col=col: self._open_upper_block_subproject_page(
                    src_row=row,
                    src_col=source_col,
                )
            )
    ...
    def _insert_row_at(self, row: int):
        ...
        for c in range(cols):
            ...
            if c == OVERALL_ASSESSMENT_COL:
                self._set_overall_assessment_combo(insert_row, "")
                continue
    ...
    def _open_upper_block_subproject_page(self, src_row: int, src_col: int | None = None):
        ...
        key = f"uppercalc::{src_row}::{src_col if src_col is not None else 9}"
        ...
        page.set_context(source_row=src_row, source_row_data=row_data, source_col=src_col)
        saved_key = (src_row, src_col if src_col is not None else 9)
        if saved_key in self._uppercalc_saved_data:
            page.load_table_data(self._uppercalc_saved_data[saved_key])
    ...
    def _on_upper_page_saved(self, payload: dict):
        ...
        src_col = payload.get("source_col")
        saved_key = (src_row, src_col if src_col is not None else 9)
        if "table_data" in payload: self._uppercalc_saved_data[saved_key] = payload["table_data"]
        calc_bg = QColor("#eef8ef")
