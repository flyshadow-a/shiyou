...
class RemoteOutlineImageExportWorker(QObject):
    finished = pyqtSignal()
    failed = pyqtSignal(str)
    ...
    def run(self) -> None:
        """
        服务端导出可行性评估报告需要的二维立面轮廓图。

        关键点：
        1. 不再在客户端本地启动 report_image_batch_export_process.py；
        2. 不再使用 QProcess + sys.executable；
        3. 打包后不会重新启动 ShiyouClient.exe；
        4. 因此不会再次弹出登录界面。
        """
        try:
            if not self._facility_code:
                raise ValueError("facility_code 不能为空，无法导出轮廓图。")

            api = ApiClient()
            task_id = api.export_images(
            ...
            if str(task.get("status") or "").lower() != "success":
                raise RuntimeError(
                    str(task.get("error") or task.get("message") or "服务端轮廓图导出失败")
                )
...
class FeasibilityAssessmentResultsPage(BasePage):
            ...
            load_information_rows.append(
                {
                    ...
                    "dry_weight_mt": str(row.get("dry_weight_mt") or "").strip(),
                    ...
                    "dry_center_xyz": str(row.get("dry_center_xyz") or "").strip(),
                    "center_xyz": str(row.get("center_xyz") or "").strip(),
                    "center_radius_m": str(row.get("center_radius_m") or "").strip(),
                    "op_fx_kn": str(row.get("op_fx_kn") or "").strip(),
                    "op_fy_kn": str(row.get("op_fy_kn") or "").strip(),
                    "op_fz_kn": str(row.get("op_fz_kn") or "").strip(),
                    "op_mx_kn_m": str(row.get("op_mx_kn_m") or "").strip(),
                    "op_my_kn_m": str(row.get("op_my_kn_m") or "").strip(),
                    "op_mz_kn_m": str(row.get("op_mz_kn_m") or "").strip(),
    ...
    def _report_image_export_command(self) -> tuple[str, list[str]]:
        ...
        旧版会返回 sys.executable + main.py / --report-image-export-worker，
        打包后 sys.executable 会变成 ShiyouClient.exe，从而重新打开登录界面。

        新版不再使用本地子进程导图，所以这里保留空返回，避免旧调用报错。
        """
        return "", []
