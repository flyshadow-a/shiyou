...
def _row_name_stem(row: dict[str, Any]) -> str:
    return Path(str(row.get("original_name", "") or "")).stem.lower()


def _row_matches_runtime_file(row: dict[str, Any], kind: str) -> bool:
    stem = _row_name_stem(row)
    if kind == "model":
        return stem.startswith("sacinp")
    if kind == "clplog":
        return stem.startswith("clplog")
    if kind == "ftglst":
        return stem.startswith("ftglst")
    if kind == "ftginp":
        return stem.startswith("ftginp")
    return False
...
def resolve_current_model_inputs(facility_code: str, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in sorted(rows, key=_row_sort_key):
        ...
        if _logical_has_segment(logical_path, "当前模型/结构模型") and _row_matches_runtime_file(current_row, "model") and not model_path:
            ...
            and _row_matches_runtime_file(current_row, "ftglst")
        ):
            ftglst_rows.append(current_row)
        elif (
            _logical_has_segment(logical_path, "当前模型/疲劳分析")
            and _logical_has_segment(logical_path, "输入")
            and _row_matches_runtime_file(current_row, "ftginp")
        ):
