...
- 2026-05-15：调整“新增特检策略”规则修正交互（涉及 `pages/new_special_inspection_page.py`）：移除页面上的四个独立规则按钮入口，将“节点分类修正 / 构件分类修正 / 构件剔除修正 / 节点剔除修正”改为点击“更新风险等级”后的串行计算步骤；任一步取消即中止本次计算，避免用户在正式计算前额外手动逐个进入四个弹窗。
- 2026-05-15：按当前特检策略需求新增四类规则修正入口：节点分类修正、构件分类修正、构件剔除修正、节点剔除修正；弹窗统一支持 4 字符通配规则、全 `*` 行占位不生效、字母/数字输入、无向构件匹配及“命中数量预览”，运行时按“剔除规则 > 人工分类修正 > VBA 自动分类 > Other”顺序生效，并保持当前流程不执行 `Sheet11.删除JOINT`。
...
- 2026-05-13：调整特检策略风险结果删除时机与规则：`pages/output_special_strategy/inspection_tool.py` 在构件风险、节点风险、节点未来疲劳风险计算完成后，按给定 VBA `删除MEMBER/删除JOINT1` 规则过滤构件、节点及其后续检验策略输出；`services/special_strategy_runtime.py` 关闭报告上下文二次删除开关，避免额外后置删除规则影响统计和报告。
- 2026-05-13：修正特检策略节点删除范围：`pages/output_special_strategy/inspection_tool.py` 注释掉 `joint_risk_df = _apply_joint_delete_rule(joint_risk_df)`，不再对 `Sheet11` 当前节点风险表执行 `删除JOINT`，仅保留 `Sheet12.删除JOINT1` 对节点预测结果及后续节点策略输出的过滤。
- 2026-05-15：按正式业务流程重排特检策略计算链路：`pages/output_special_strategy/inspection_tool.py` 将自动分类顺序调整为 `Sheet3.Member -> Sheet2.JointTYPE`，并把流水线拆为“分类修正后执行第 6/7/8 步”和“第 8 步后执行构件/节点剔除及 `Sheet10/Sheet12` 等效过滤”两阶段；`services/special_strategy_runtime.py` 与 `pages/new_special_inspection_page.py` 同步改为前置分类修正弹窗、阶段性风险计算、后置剔除修正弹窗、最终落盘的两段式交互，确保剔除规则命中预览基于真实第 8 步结果而非原始模型全集。
- 2026-05-15：补充特检策略四类规则写回审计：`pages/output_special_strategy/inspection_tool.py` 新增 `RuleOverrideAudit` 输出表，记录用户在“新增构件分类修正 / 新增节点分类修正 / 剔除不考虑构件修正 / 剔除不考虑节点修正”四个窗口中设置的规则实际命中的对象、改写前后值及是否发生变化；前置分类审计基于 `Members/Joints` 的真实改写结果，后置剔除审计基于第 8 步后的 `MemberRisk/JointForecast` 结果，便于确认窗口输入已正确写回计算数据。
- 2026-05-18：修复特检策略历史运行记录读取 UNC 路径时报 `WinError 1326` 的问题：`services/special_strategy_runtime.py` 新增不触发网络解析的路径构造辅助函数，`_state_from_run_payload`、`_artifact_dir_from_state` 及历史结果/报告回读路径不再在仅需拆分父目录时调用 `Path.resolve()`，避免对未认证共享目录进行隐式访问。
- 2026-05-18：完善特检策略四个规则修正弹窗 UI：`pages/special_strategy_rule_dialogs.py` 将“新增节点分类修正 / 新增构件分类修正 / 剔除不考虑构件修正 / 剔除不考虑节点修正”统一为正式弹窗样式，采用分组卡片、浅色表头、固定“取消 / 确认”按钮顺序和命中预览提示条；保留原有规则匹配、冲突确认与写回逻辑不变。
- 2026-05-18：完善“新增特检策略”设置等级参数交互：`pages/new_special_inspection_page.py` 将“生命安全等级 / 失效后果等级 / 平台整体暴露等级”的值列改为下拉选择，支持 `S-1~S-3`、`C-1~C-3`、`L-1~L-3`，并让说明列随 `S/C` 选择联动更新；参数采集改为优先读取下拉控件当前值，确保计算使用用户最新选择。
- 2026-05-18：重整“新增特检策略”结构模型信息区：`pages/new_special_inspection_page.py` 去掉模型参数表和工作点坐标表中原有亮黄色输入底色，将区域拆成“模型参数 / 腿柱工作点坐标”两段，并为坐标表新增“新增一行 / 删除选中行”操作；显式工作点列表按实际数量展示，增删后自动重排序号并同步腿柱数量，计算参数以有效坐标行数为准。
- 2026-05-18：继续收口特检策略页面交互（涉及 `pages/special_inspection_strategy.py`、`pages/new_special_inspection_page.py`）：恢复顶部平台筛选下拉栏，并在切换设施后重载对应平台参数与文件；左右主区域宽度比对齐结构强度页为 `7:4`；生命安全等级、失效后果等级、平台整体暴露等级改为初始空白下拉并统一样式，去除平台暴露等级红底；“设置等级参数”表初始行高统一；腿柱工作点坐标表取消独立增删入口和内部滚动，行数改为跟随“腿柱数量”自动同步并完整显示在左侧区域；同时修复 `_SpecialStrategyCalculationWorker` 方法误落到类外的合并残留问题。
- 2026-05-18：修复特检策略规则弹窗“新增一行”按钮回调异常（涉及 `pages/special_strategy_rule_dialogs.py`）：统一在按钮封装层吞掉 `QPushButton.clicked(bool)` 传入的布尔参数，避免其覆盖成员规则表格回调中的默认参数并触发 `'bool' object has no attribute 'rowCount'`；已对四类规则弹窗的新增行按钮做离屏交互回归。
- 2026-05-18：继续修正“新增特检策略”左侧交互与视觉（涉及 `pages/new_special_inspection_page.py`）：将腿柱坐标表首列表头恢复为“腿柱工作点坐标”，坐标表保持在“结构模型信息”同一左侧卡片内完整展示并删除独立滚动辅助逻辑；修复固定展示表格在腿柱数量变化时被 `resizeRowsToContents()` 重新压缩导致的高度跳变，改为固定行高后按当前行高计算总高度；同步将左侧区块改为统一白底卡片、优化表头与选中态样式，并把等级下拉初始态改为显示 `▼`、实际取值仍为空的占位项，避免占位符写回计算参数。
- 2026-05-18：继续清理特检策略页面视觉噪音（涉及 `pages/new_special_inspection_page.py`、`pages/special_strategy_rule_dialogs.py`）：将等级参数选择从单元格内嵌 `QComboBox` 改为“单元格内显示 `▼`，点击后弹出菜单”的表格原生交互，避免小控件浮在文字区上方；文件设置表“序号”列宽与“腿柱工作点坐标”列统一；移除四类规则弹窗预览提示和表头单元格的彩色底，以及文件设置标题行的浅色底，统一改为更干净的白底表达。
- 2026-05-18：继续收敛特检策略规则弹窗文字样式（涉及 `pages/special_strategy_rule_dialogs.py`）：将标题、提示、分组标题、预览说明和底部说明改为控件级透明背景，避免被外层页面 `QWidget` 背景样式重新染色；同时让预览文字在动态变色时继续保留透明背景，去掉文字后的色块感。
- 2026-05-18：优化“新增特检策略”等级参数下拉菜单悬停态（涉及 `pages/new_special_inspection_page.py`）：为生命安全等级、失效后果等级、平台整体暴露等级的弹出菜单补充统一 QSS，菜单项悬停高亮由接近白色调整为浅蓝色，增强可见性并与仓库内其他菜单风格保持一致。
- 2026-05-20：改造特检策略构件规则表达（涉及 `pages/special_strategy_rule_dialogs.py`、`pages/output_special_strategy/inspection_tool.py`）：新增构件分类修正与剔除不考虑构件修正弹窗的成员规则表增加“关系”下拉列，支持 `And/Or/Not`；无既有规则时预填主腿构件、X撑构件和剔除构件默认规则；计算内核同步保存并执行关系字段，构件匹配按节点 A/B 与 B/A 双向遍历，不再依赖模型中节点顺序固定。
- 2026-05-20：取消特检策略构件规则弹窗自动预填（涉及 `pages/special_strategy_rule_dialogs.py`）：保留构件规则表“关系”下拉列与计算内核 `And/Or/Not` 支持，但新增构件分类修正和剔除不考虑构件修正默认只显示全 `*` 占位行，确认前不产生有效构件规则，避免默认内容被误认为用户已确认规则。
- 2026-05-20：修复特检策略规则弹窗新增行后内容不可见的问题（涉及 `pages/special_strategy_rule_dialogs.py`）：将弹窗主体放入内部滚动区并固定底部“取消/确认”按钮；表格新增行时弹窗最多自动增高 5 行表格高度，超过阈值后保持当前最大高度并通过内部垂直滚动查看下方内容。
- 2026-05-20：修复“新增特检策略”页面重启后自动带入上次运行参数的问题（涉及 `pages/new_special_inspection_page.py`）：页面默认参数加载由 `load_latest_strategy_params()` 改为 `load_default_params()`，避免数据库最新运行记录中的 `params_json` 回填到新增页面；历史运行参数仍保存在数据库中供结果/报告链路使用。
- 2026-05-21：修正“新增特检策略”设置等级参数下拉菜单弹出位置（涉及 `pages/new_special_inspection_page.py`）：将生命安全等级、失效后果等级、平台整体暴露等级菜单定位从单元格左下角改为值单元格右下角对齐，使菜单贴近箭头下方展开，减少滚动区/表格宽度变化造成的位置偏移感。
- 2026-05-20：调整特检策略结构分类默认流程（涉及 `pages/output_special_strategy/inspection_tool.py`）：将 `classify_structure`、`prepare_run_state`、`run` 中的 `apply_sheet2_jointtype` 与 `apply_sheet3_membertype` 默认值由 `True` 改为 `False`，默认不再执行 Sheet2/Sheet3 命名规则覆盖，保留前置拓扑识别和人工分类修正链路。
- 2026-05-20：恢复特检策略结构分类默认流程（涉及 `pages/output_special_strategy/inspection_tool.py`）：按当前需求将 `classify_structure`、`prepare_run_state`、`run` 中的 `apply_sheet2_jointtype` 与 `apply_sheet3_membertype` 默认值改回 `True`，默认继续执行 Sheet2/Sheet3 命名规则覆盖。
